...
public class CJframe_test extends JFrame {
	...
	public CJframe_test() {
		...
		btnNextpage.addActionListener(new ActionListener() {//下一頁
			public void actionPerformed(ActionEvent arg0) {
				if(pageNumber<(maxpage/10)+1){//最大頁碼
					pageNumber++;
					arrClear(arr);
					jdbcsql.SelectTable(arr, pageNumber);//從資料庫拿10筆資料放到arr ,pageNumber為參照值 決定哪10筆
					textArea.setText("Number\tdate\tid");
					for(int i=0;i<10;i++){
						if(arr[i][0]==null)
							break;
